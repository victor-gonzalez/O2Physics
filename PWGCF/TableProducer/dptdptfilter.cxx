...
struct DptDptFilter {
  ...
  Configurable<o2::analysis::PWGCF::DptDptBinningCuts> cfgBinning{"binning",
                                                                  {28, -7.0, 7.0, 18, 0.2, 2.0, 16, -0.8, 0.8, 72, 0.5},
                                                                  "triplets - nbins, min, max - for z_vtx, pT, eta and phi, binning plus bin fraction of phi origin shift"};
  Configurable<o2::analysis::PWGCF::CheckRangeCfg> cfgTraceDCAOutliers{"trackdcaoutliers", {false, 0.0, 0.0}, "Track the generator level DCAxy outliers: false/true, low dcaxy, up dcaxy. Default {false,0.0,0.0}"};
...
bool DptDptFilter::selectTrack(TrackObject const& track, int64_t colix)
{
  ...
  if ((asone == uint8_t(true)) or (astwo == uint8_t(true))) {
    ...
    if (sp != kWrongSpecies) {
      ...
      scannedtracks(colix, asone, astwo, uint64_t(0), track.pt(), track.eta(), track.phi());
      return true;
    }
  }
  return false;
    ...
    if (charge != 0) {
      ...
      if ((asone == uint8_t(true)) or (astwo == uint8_t(true))) {
        ...
        if (sp != kWrongSpecies) {
          ...
          scannedtruetracks(colix, asone, astwo, uint64_t(0), particle.pt(), particle.eta(), particle.phi());
          acceptedparticles++;
        }
      }
    } else {
...
void DptDptFilter::processReconstructed(CollisionObject const& collision, TracksObject const& ftracks, float passedcent)
{
  ...
  if (IsEvtSelected(collision, centormult)) {
    ...
    acceptedcollisions(collision.bcId(), collision.posZ(), acceptedevent, centormult, uint64_t(0));
    ...
    acceptedcollisions(collision.bcId(), collision.posZ(), acceptedevent, centormult, uint64_t(0));
    for (auto& track : ftracks) {
      scannedtracks(acceptedcollisions.lastIndex(), uint8_t(false), uint8_t(false), uint64_t(0), track.pt(), track.eta(), track.phi());
    }
  }
}
...
void DptDptFilter::processGenerated(CollisionObject const& mccollision, ParticlesList const& mcparticles, float centormult)
{
  ...
  if (IsEvtSelected(mccollision, centormult)) {
    acceptedevent = true;
    acceptedtrueevents(mccollision.bcId(), mccollision.posZ(), acceptedevent, centormult, uint64_t(0));
